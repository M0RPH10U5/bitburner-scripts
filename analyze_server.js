export async function main(ns) {
    ...
    const fractionalDigits = "$0.000a".replace(/[^0-9]/g, "").length;
    ns.tprint(`

${server}:
    RAM        : ${ram[1]} / ${ram[0]} (${ram[1] / ram[0] * 100}%)
    $          : ${ns.formatNumber(money, fractionalDigits)} / ${ns.formatNumber(maxMoney, fractionalDigits)} (${(money / maxMoney * 100).toFixed(2)}%)
